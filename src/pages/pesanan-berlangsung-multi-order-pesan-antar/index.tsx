...
import map from '@assets/images/MapTrack.png';
...
const MultiOrderInProgressPesanAntar: PageComponent = () => {
  ...
  const [openModalSiapkanPesanan, setOpenModalSiapkanPesanan] = useState(false);
  const [openModalDriverFail, setopenModalDriverFail] = useState(false);
  ...
  const toggleOpenModalSiapkanPesanan = () => {
    setOpenModalSiapkanPesanan(!openModalSiapkanPesanan);
  };
  const toggleOpenModalDriverFail = () => {
    setopenModalDriverFail(!openModalDriverFail);
  };
  ...
  useEffect(() => {
    ...
    const totalMenuCost = orders.reduce((total, order) => {
 ...
 <Grid container={true} spacing={2} sx={{ opacity: cekDriver ? 1 : 0.5, marginBottom: '1rem' }}>
        <Grid
          item={true}
          sx={{
            alignItems: 'center',
          ...
          xs={2}
        >
        <Avatar src={cekDriver ? OrangJatnika : ''} sx={{ height: '50px', width: '50px' }}>
        {cekDriver ? null : <PersonIcon />}
        </Avatar>
        </Grid>
        <Grid item={true} sx={{ alignItems: 'center', display: 'flex', justifyContent: 'start', paddingTop: '0rem!important' }} xs={8}>
          <Box>
            <Typography
              ...
              variant="h6"
            >
            {cekDriver ? 'Jatnika \u2022 D 1993 XD' : ''}
            </Typography>
            <Typography variant="body2">
            {cekDriver ? 'Driver Ditemukan' : 'Mencari Driver'}
            </Typography>
          </Box>
        </Grid>
    ...
    <Box>
      ...
      {DUMMY_PESANAN.map((order) => (
      <Accordion key={order.restoId} sx={{ marginBottom: '1rem', opacity: order.orderApproved ? 1 : 0.5 }}>
        <AccordionSummary
      ...
      <Modal
        aria-describedby="modal-modal-description"
        aria-labelledby="modal-modal-title"
        open={openModalDriverFail}
        onClose={toggleOpenModalDriverFail}
      >
        <Box sx={style}>
          <Typography
            component="h3"
...
MultiOrderInProgressPesanAntar.displayName = 'MultiOrderInProgressPesanAntar';

export default MultiOrderInProgressPesanAntar;
